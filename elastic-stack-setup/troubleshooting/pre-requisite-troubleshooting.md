...
| Environment Watch Installer | 389 | LDAP | | ✅ | Outbound connection from the target server to a domain controller, used to validate the Relativity Service Account during install/upgrade only — this port is not required during normal runtime. See [Domain Controller Unreachable](monitoring-agent-and-otel-collector.md#domain-controller-unreachable) if the installer reports a service-account validation error. |
