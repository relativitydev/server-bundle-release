...
| Environment Watch Installer | 389 | LDAP | | ✅ | Outbound connection from the target server to a domain controller, used to validate the Relativity Service Account during install/upgrade only — not required during normal runtime. Port 3268 is not a provisioning requirement; see [Domain Controller Unreachable](monitoring-agent-and-otel-collector.md#domain-controller-unreachable) if the installer reports it. |
