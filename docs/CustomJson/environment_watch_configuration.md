...
| `acknowledgeAlertEnabled`  | Boolean flag to enable/disable alert acknowledgment in Slack.                |
| `channel`                  | The Slack channel name where alerts will be posted.                         |
| `enabled`                  | Boolean flag to enable/disable Slack notifications.                         |
| `messageIntervalSeconds`   | Interval (in seconds) between alert messages sent to Slack.                 |
	...
	"environmentWatchConfiguration": {
		...
		"alertNotificationHandlers": {
			"slack": {
				...
				"channel": "alert-server-unit-test",
				"enabled": true,
