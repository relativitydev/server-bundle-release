...
2. Verify if Elasticsearch accessible from a given host:
	a. Check if elasticsearch is accessible by accessing https://emttest:9200/
...
3. Verify the SSL/TLS URL
    ...
    b. Check Elasticsearch elasticsearch.yml configuration and verify that SSL/TLS settings are correctly defined.
...
2. Go to that directory where you saved the certificate. Double click on the certificate and then click Install Certificate
...
8. Open MMC, go to files and click on Add/Remove Snap-IN
...
13. Right Click on Trusted Root Certification Authorirties, All Task → Import
...
15. Import certificate for all the selected folders below.

   ![alt text](../resources/troubleshooting-images/importcerts.png)

16.  Close all search engine and re-login to ElasticSearch URL
...
1. Verify Kibana Service Status:
    a. Run the service and verify if service is up and running 
...
2. In the kibana.yml configuration file, add the xpack.encryptedSavedObjects.encryptionKey setting.
...
1. Verify APM Service Status
    a. Check if apm server is running using powershell command
    ...
    c. Check if apm is running by navigating to APM URL http://emttest:8200/ in any supported Web browser.
...
3. If server is not whitelisted, whitelist servers on secretstore to grant permission by running the below command:
...
**Troubleshooting Steps:**
1. Verify if the one-time setup using the relsvr.exe CLI was executed properly and ensure that it has been completed as required.
...
2. If unable to login to relativity, Check the Relativity services in IIS and start them
